...
task main()
{
	...
	while (true)
	{
		...
		if (Tilter==true)
		{
			if (time1[T1]<500)//if the current time is less then .5 sec
			{
				motor [DumpMotorLeft]= 75;
			}
			else
			{
				motor [DumpMotorLeft]= 0;
		...
		if (joy2Btn (5)==true)
		{
			servo [Hook] = 0;			// This will put the "hook" down onto the ball holder, keeping it by us.
		}
		if (joy2Btn (6)==true)
		{
			servo [Hook] = 180;			// This will lift the "hook", releasing the ball holder.
		}
	}
}
